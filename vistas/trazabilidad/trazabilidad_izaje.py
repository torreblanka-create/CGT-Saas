...
def render_trazabilidad_izaje(DB_PATH):
    categoria_seleccionada = "Elementos de izaje"
    filtros = st.session_state.get('filtros', {'empresa_id': 0, 'empresa_nom': None, 'contrato_id': 0, 'contrato_nom': None, 'busqueda_global': ''})
    st.markdown(f"<h2 style='color: var(--cgt-blue);'>🛡️ Trazabilidad: Elementos de Izaje</h2>", unsafe_allow_html=True)
    ...
    opciones_doc = ["Certificado de operatividad (Trimestral)", "Memoria de cálculo", "Inspección visual", "Otros"]
    ...
    with st.container(border=True):
        ...
        if not df_creados.empty:
            ...
            seleccion_rapida = st.pills("Selecciona un elemento de izaje ya registrado:", df_creados['pildora'].tolist(), key="pills_izaje")
        else:
            st.info("Aún no hay elementos de izaje con expedientes en esta base de datos.")
        ...
        if seleccion_rapida:
            ...
            if df_f.empty:
                ...
                with st.expander(f"⚡ Sincronización Masiva (Cargar a todos desde el Excel)"):
                    ...
                    if st.button(f"🚀 Crear Tarjetas para TODOS en Izaje", use_container_width=True):
            ...
            if not is_valid_context(filtros):
                ...
                st.info(f"📝 Ingreso manual de datos para: Elementos de Izaje")
    ...
    if id_final and nombre_final:
        st.divider()
        st.markdown(f"### 🪪 Perfil del Elemento", unsafe_allow_html=True)
                ...
                if docs_a_cargar:
                    ...
                    for doc in docs_a_cargar:
                        ...
                        with st.expander(f"📄 Configurar: {doc}", expanded=True):
                            ...
                            if doc in ["Certificado de operatividad (Trimestral)"]:
