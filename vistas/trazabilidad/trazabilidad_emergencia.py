...
def render_trazabilidad_emergencia(DB_PATH):
    categoria_seleccionada = "Sistemas de Emergencia"
    filtros = st.session_state.get('filtros', {'empresa_id': 0, 'empresa_nom': None, 'contrato_id': 0, 'contrato_nom': None, 'busqueda_global': ''})
    st.markdown(f"<h2 style='color: var(--cgt-blue);'>🛡️ Trazabilidad: Sistemas de Emergencia</h2>", unsafe_allow_html=True)
    ...
    opciones_doc = ["Revisión técnica anual", "Prueba hidrostática", "Inspección visual mensual", "Otros"]
    ...
    with st.container(border=True):
        ...
        if not df_creados.empty:
            ...
            seleccion_rapida = st.pills("Selecciona un sistema de emergencia ya registrado:", df_creados['pildora'].tolist(), key="pills_emcg")
        else:
            st.info("Aún no hay sistemas de emergencia con expedientes en esta base de datos.")
        ...
        if seleccion_rapida:
            ...
            if df_f.empty:
                ...
                with st.expander(f"⚡ Sincronización Masiva (Cargar a todos desde el Excel)"):
                    if not is_valid_context(filtros):
                        ...
                        if st.button(f"🚀 Crear Tarjetas para TODOS en Sistemas de Emergencia", use_container_width=True):
            ...
            if not is_valid_context(filtros):
                ...
                st.info(f"📝 Ingreso manual de datos para: Sistemas de Emergencia")
    ...
    if id_final and nombre_final:
        st.divider()
        st.markdown(f"### 🪪 Perfil del Sistema", unsafe_allow_html=True)
                ...
                if docs_a_cargar:
                    ...
                    for doc in docs_a_cargar:
                        ...
                        with st.expander(f"📄 Configurar: {doc}", expanded=True):
                            ...
                            if doc in ["Revisión técnica anual", "Prueba hidrostática"]:
